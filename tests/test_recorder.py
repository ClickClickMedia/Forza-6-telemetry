...
def test_auto_session_on_raceon(tmp_path):
    ...
    # keep_min_s=0: these tests exercise lifecycle mechanics with tiny
    # sessions; the blip filter has its own tests in test_housekeeping.py.
    rec = Recorder(db, tmp_path / "sessions", idle_timeout_s=5.0,
                   raw_format="csv", keep_min_s=0.0, record_mode="motion")
...
def test_manual_recording_and_marker(tmp_path):
    db = Database(tmp_path / "s.db")
    rec = Recorder(db, tmp_path / "sessions", raw_format="csv", keep_min_s=0.0, record_mode="motion")
