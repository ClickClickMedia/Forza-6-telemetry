...
async def session_tuning_md(session_id: int, download: int = 0,
    ...
    md = build_markdown(sd, row, __version__, setup=setup,
                        ...
                        variant=(mode if mode in ("data", "quick")
                                 else "full"),
...
async def session_package(session_id: int) -> Response:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("START-HERE.md", build_package_index(
            row, int(row.get("frame_count") or 0),
            has_raw=has_raw, has_setup=setup is not None))
        zf.writestr("report.md", md)
        zf.writestr("corner-events.json", json.dumps(sec, indent=1))
        zf.writestr("laps.csv", build_laps_csv(sd))
        zf.writestr("session-info.json", json.dumps(metadata, indent=1,
                                                    default=str))
        if setup:
            zf.writestr("your-tune.json", json.dumps(setup, indent=1))
        if has_raw:
            zf.write(raw_path, "raw-telemetry.csv")
    cls = {0: "D", 1: "C", 2: "B", 3: "A", 4: "S1", 5: "S2", 6: "S3",
           7: "X"}.get(row.get("car_class"), "")
    tag = "-".join(x for x in (cls, str(row.get("car_pi") or "")) if x)
    fname = "_".join(x for x in (
        "FH6", _safe(row.get("car_name") or row.get("name") or "session"),
        tag, (row.get("created_at") or "")[:10]) if x) + ".zip"
    return Response(
