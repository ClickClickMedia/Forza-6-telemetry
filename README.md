...
1. **Drive.** Recording is automatic. Laps in a race, Rivals or Time Trial
   give the richest data (lap times populate there); free roam works too.
2. Open **Sessions → Export → "Copy tuning report"** (or the same button on
   ...
   fill-in block for your current setup values and a prompt that tells the
   AI exactly how to read the numbers.
3. **Paste it into Claude or ChatGPT** (free tiers work). It will propose
   changes within Forza's tuning screen: pressures, gearing, alignment,
   anti-roll bars, springs, damping, aero, diff, brakes.
...
- **Recording** — auto-starts when you drive, auto-ends after 5 s of
  silence, plus manual Start/Stop/Marker controls. Raw frames stored as
  portable CSV with monotonic timestamps; session metadata in SQLite.
  Rename, annotate, download — all from the phone.
...
- All data stays local — recordings live in `./data` on your machine. There
  is no telemetry, analytics, or cloud call anywhere in this project.
